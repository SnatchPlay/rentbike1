...
void PlaceRep::ReadFromStorage()
{
    ...
    str1.open("C:/Users/Ûלרפוכש/source/repos/Project1/str1.txt");
    if (!str1)
    {
...
{
    ofstream str;
    str.open("C:/Users/Ûלרפוכש/source/repos/Project1/str1.txt");
    int tmp = entity.size();
    for (int i = 0; i < tmp; i++) {

        str << entity[i].getstreet() << " " << (entity[i]).sr() << " " << (entity[i]).pri() << " " << (entity[i]).avall() << endl;
    }
    str.close();
}
